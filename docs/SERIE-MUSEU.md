...
| 10 | Onda 3 — buracos coronais e plumas | Marcador semântico `coronaHoleDir`/`strength`/`generation` publicado no bake atômico do volume coronal; cartão LOCAL com âncora coronal (1.35R), coleção 10→11, prova white-box (`eduCoronaHoleState`) + negativo duplo (`cvol=0` indisponível; máximo com coroa cheia abaixo do corte) | `npm run qa:ci` verde + prova white-box do marcador + controle negativo | "No mínimo do ciclo, as regiões escuras da coroa ganham nome." | 🚧 em andamento |
